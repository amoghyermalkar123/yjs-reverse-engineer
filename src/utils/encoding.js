...
export const readClientsStructRefs = (decoder, doc) => {
  ...
  for (let i = 0; i < numOfStateUpdates; i++) {
    ...
    // create the clients map of ID to block list
 ...
 * @param {Item | GC} ref
 */
const printBlockToIntegrate = (ref) => {
  // Always print the id, which is common to both Item and GC
  let output = `block to integrate: id=${ref.id.client}:${ref.id.clock}`;

  // Check if ref is an instance of Item to print additional fields
  if (ref instanceof Item) {
    output += `, origin=${ref.origin}, left=${ref.left}, right=${ref.right}, rightOrigin=${ref.rightOrigin}, content=${ref.content.getContent()}`;
  } else {
    output += `, This is a GC object, no additional fields.`;
  }
...
/**
 * @param {Map<number, { i: number, refs: Array<Item | GC> }>} myMap
 *
 * @private
 * @function
 */
 ...
 * Decode the binary update, perform integration on a per-block basis
